...
function create({ name, directory, mode, provider }) {
  validateName(name);
  ...
  if (mode !== 'bash') {
    // Inject GLM 5.1 env vars if using z.ai provider
    if (provider === 'glm') {
      const zaiKey = process.env.ZAI_API_KEY || '';
      execF('tmux', ['send-keys', '-t', name,
        `export ANTHROPIC_API_KEY=${zaiKey} ANTHROPIC_AUTH_TOKEN=${zaiKey} ANTHROPIC_BASE_URL=https://api.z.ai/api/anthropic ANTHROPIC_DEFAULT_SONNET_MODEL=glm-5.1 ANTHROPIC_DEFAULT_OPUS_MODEL=glm-5.1 CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1`,
        'Enter']);
    }
    execF('tmux', ['send-keys', '-t', name, 'claude --dangerously-skip-permissions', 'Enter']);
